...
enum DesfireCommand : uint8_t {
    ...
    DF_CMD_GET_ADDITIONAL_FRAME      = 0xAF, ///< Get additional data if command response doesn't fit into buffer
...
struct DESFireCardVersion {
    ...
    const char* getCardTypeName() const {
        switch (hardwareType) {
            ...
            case 0x41: return "DESFire Light";
    ...
    uint32_t getStorageSize() const {
        switch (softwareStorageSize) {
            case 0x00: return 2048;    // 2K
            case 0x01: return 4096;    // 4K
            case 0x02: return 8192;    // 8K
            case 0x03: return 16384;   // 16K
            case 0x04: return 32768;   // 32K
            case 0x0D: return 131072;  // 128K (Light)
